...
11. `Human Review` へ動かす前に CI と feedback の close-out loop を回す
    ...
    - PR の Draft / Ready 状態を `gh pr view --json isDraft` で確認する。Draft なら `gh pr ready` を実行して Ready 化する。`ready_for_review` event を発火させ Slack 通知をレビュアー へ届けるため
12. 上記を満たしたら `mcp__linear-mh4gf__save_issue` で `Human Review` へ動かす
    ...
    - その時は blocker brief と unblock action を workpad に書いた上で `Human Review` へ動かす。PR は Draft のままで構わない
...
- escape hatch で遷移する場合、PR は Draft のままで構わない。Completion bar の PR Ready 必須要件は本 hatch にのみ適用しない
...
- PR が Ready 状態 (Draft でない) で `ready_for_review` event が発火している (escape hatch で遷移する場合は例外)
